...
# # Radar Plot
# 
# ggplot(head(wf_uni, 50), aes(x = term, y = count, fill = term)) + geom_bar(
#   width = 0.75,
#   stat = "identity",
#   colour = "black",
#   size = 1
# ) + coord_polar(theta = "x") + xlab("") + ylab("") + ggtitle("Word Frequency") + theme(legend.position = "none") + labs(x = NULL, y = NULL)
# 
# # Histograms
# 
# p1 <-
#   ggplot(head(wf_uni, 20), aes(reorder(term, -count), count)) + geom_bar(stat =
#                                                                            "identity") +
...
#                                                                                    count), vjust = -1, size =
#                                                                                3) +
#   ylim(0, round(max(wf_uni$count) + 0.05 * max(wf_uni$count)))
# p1
# 
# p2 <-
#   ggplot(head(wf_bi, 20), aes(reorder(term, -count), count)) + geom_bar(stat =
#                                                                           "identity") +
#   theme(axis.text = element_text(size = 10),
#         axis.title = element_text(size = 10, face = "bold")) +
#   theme(axis.text.x = element_text(
...
#                                                                                    count), vjust = -1, size =
#                                                                                3) +
#   ylim(0, round(max(wf_bi$count) + 0.05 * max(wf_bi$count)))
# p2
# 
# p3 <-
#   ggplot(head(wf_tri, 20), aes(reorder(term, -count), count)) + geom_bar(stat =
#                                                                            "identity") +
#   theme(axis.text = element_text(size = 10),
#         axis.title = element_text(size = 10, face = "bold")) +
#   theme(axis.text.x = element_text(
...
#                                                                                    count), vjust = -1, size =
#                                                                                3) +
#   ylim(0, round(max(wf_tri$count) + 0.05 * max(wf_tri$count)))
# p3
# 
# p4 <-
#   ggplot(head(wf_quad, 20), aes(reorder(term, -count), count)) + geom_bar(stat =
#                                                                             "identity") +
#   theme(axis.text = element_text(size = 10),
#         axis.title = element_text(size = 10, face = "bold")) +
#   theme(axis.text.x = element_text(
...
#                                                                                    count), vjust = -1, size =
#                                                                                3) +
#   ylim(0, round(max(wf_quad$count) + 0.05 * max(wf_quad$count)))
# p4

# ## remove all counts lower n < N for efficiency but store the counts for smoothing
# 
# # count lower order ngrams
# 
# wf_uni_lc <- as.data.table(table(wf_uni[wf_uni$count <= 1,]$count))
# wf_bi_lc <- as.data.table(table(wf_bi[wf_bi$count < 2,]$count))
# wf_tri_lc <- as.data.table(table(wf_tri[wf_tri$count < 3,]$count))
# wf_quad_lc <- as.data.table(table(wf_tri[wf_tri$count < 4,]$count))
# 
...
##### Create Stupid Backoff functions
...
# ## perform kneser ney on the ngrams
#  
# wf_uni$p_mkn <- mkney_uni(wf_uni)
# wf_bi$p_mkn <- mkney_bi(wf_bi)
# wf_tri$p_mkn <- mkney_tri(wf_tri)
# wf_quad$p_mkn <- mkney_quad(wf_quad)

## perform stupid backoff

listdt <- stu_bo(wf_uni,wf_bi,wf_tri,wf_quad)
...
wf_uni <- wf_uni[wf_uni$frequency > 1,]
wf_bi <- wf_bi[wf_bi$frequency >= 2,]
wf_tri <- wf_tri[wf_tri$frequency >= 3,]
wf_quad <- wf_tri[wf_tri$frequency >= 4,]
...
#   quad <- head(wf_quad[wf_quad$FirstTerms==trigram,c('LastTerm','p_mkn')])
#   bigram <- paste(tail(winput,2),collapse = ' ')
#   tri <- head(wf_tri[wf_tri$FirstTerms==bigram,c('LastTerm','p_mkn')])
#   unigram <- tail(winput,1)
#   bi <- head(wf_bi[wf_bi$FirstTerms==unigram,c('LastTerm','p_mkn')])
#   uni <- head(wf_uni[,c("LastTerm","p_mkn")])
#   result <- rbind(quad,tri,bi,uni)
#   arrange(head(result,10), -p_mkn)
# }
